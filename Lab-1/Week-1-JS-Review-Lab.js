...
{
   const removeExtra = parseInt(
    listings.price.replace(/[$,]/g, "")
  );

  return removeExtra <= 450000
  ? removeExtra * 0.025
  : removeExtra * 0.02;
};
...
const listingAscendingly = listings.sort(
  (listingsA, listingsB) => parseInt(listingsA.price.replace(/[$,]/g, "")) - parseInt(listingsB.price.replace(/[$,]/g, ""))
);
...
const filteredData = listings.filter((listings) => parseInt(listings.propertySummary.builtIn) >= 1990);
